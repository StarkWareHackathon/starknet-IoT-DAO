...
func get_costs{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}() -> (
    ...
    _get_array_uint256(length, array, mapping_ref)
    return (length, array)
end
...
func _check_valid_costs{pedersen_ptr : HashBuiltin*, syscall_ptr : felt*, range_check_ptr}(
    ...
    let index = costs_len - 1
    let is_less_than_max_cost : felt = uint256_lt(costs[index], max_cost)
    if is_less_than_max_cost == TRUE:
        return (0)
    end

    let is_rating_le_than_max : felt = is_le(rating_breaks[index], max_rating)
    ...
    return _check_valid_costs(
        costs_len-1, costs, rating_breaks, costs[index], rating_breaks[index])
end
...
# ## Internal function
func increment_round{pedersen_ptr : HashBuiltin*, syscall_ptr : felt*, range_check_ptr : felt}() -> ():
